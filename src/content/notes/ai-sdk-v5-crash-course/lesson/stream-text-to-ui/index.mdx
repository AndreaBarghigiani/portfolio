...
```
Now that we get `messages` and `sendMessage` from `useChat`, the only thing that we still need to do is to wire the `submit` event with the `sendMessage` function so we are able to send the message(s) that our user has typed (and the responses our LLM provided with previous answers).
...
```
As we can see, each object is full of information! We have the `role` (useful value to let our UI understand who's *'talking'*) but most importantly we have the `parts` array that will allow us to **connect different parts of a text** and build the response in our UI.
...
Since we're using AI SDK this part is pretty simple, all we have to do is to import the <EL href="https://ai-sdk.dev/docs/reference/ai-sdk-ui/convert-to-model-messages#converttomodelmessages">`convertToModelMessages`</EL> function and it'll take care of this.
```ts
