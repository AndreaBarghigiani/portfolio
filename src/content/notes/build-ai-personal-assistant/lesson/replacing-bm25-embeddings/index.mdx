...
```
As you can see we, possibly, get all the `embeddings` from our cache via `getExistingEmbeddings` (the function we described earlier to handle the generation and retrival of the embeddings from the cache).

We then generate the `emailsMap` to simplify the retrival of the content of a specific email and then, once we get the embedding of the user query, we start our loop where we calculate the similarities between the embed email and the embed user query.
