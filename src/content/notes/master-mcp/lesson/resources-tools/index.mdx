...
Since we could have hundreds of entries, and each could be incredibly long, instead of fill the context of our LLM we decide that's better to show a simple title that links to a specific resource. Then the LLM will be able to decide what to do with it. 

In order to do so we need to change the `return` inside our `map`s, and if you notice the changes that are present inside the various `content` arrays that each callback `return`, wait for the deep dive later on because Kent has done a terrific job in teaching us how to better organize our code.

And the first change we have to make is to tell that we're not returning anymore an object with the type of `resource` and instead we aim to return a `resource_list`.

Changing the type of our object requires also a different structure, one where we do not have a `resource` key describing our resource but we can define every key right at the same level.
```ts
agent.server.registerTool(
  'list_tags',
  {
...
```
So now our `tagLinks` array will be an array of `resource_link` objects where we can tell the LLM the exact `uri` for the specific resource, its `name` and `description` and, as we discovered a while ago, we can especially specify the `mimeType` so the LLM know how to treat the resource.
...
As you can see by yourself, the inspector is able to understand that we're generating a list of resources and it allow us also to dive deeper and discover its content by clicking on the URI.
