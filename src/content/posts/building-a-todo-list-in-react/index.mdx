...
export default function ToDoList() {
  ...
  return (
    <div>
      ...
      <ul>
        {tasks.map((task, i) => (
          <TaskItem key={i} taskIndex={i} text={task} handleDelete={handleDelete} />
        ))}
      </ul>
    </div>
  );
...
function TaskItem({ text, taskIndex, handleDelete }) {
  return (
    <li>
      <span>{text}</span>
      <button onClick={() => handleDelete(taskIndex)} className="border bg-slate-300 px-2 py-1">
        Delete
      </button>
    </li>
  );
...
You can discover more about `aria-live` inside the [specific MDN page,](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Attributes/aria-live) you can read it all and discover in detail how the screen readers will be able to announce updates inside this kind of container. But since the proposed solution does not even give us an example, justifying it for lack of time, here's the code I would implement during an interview.
