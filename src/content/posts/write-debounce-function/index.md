...
---
I suppose you already know why we use `debounce`, but just as a little refresher, here's my own definition: _"we use_ `debounce` _when we do not want to fire a function as soon as it gets called, but instead, we want to handle the cases when the function gets called multiple times in a short timeframe and have our program run only once”_.
